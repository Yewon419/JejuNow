...
**🔄 아이패드 실기기 넘침 — 미해결 추적 중 (2026-07-23 저녁):**
- WebKit 그리드 min-w-0 수정 후에도 실기기 증상 지속(재설치도 무효 신고).
  데스크톱 WebKit·iPad Pro 11 프로파일(뷰포트 834·모바일 시맨틱) 모두 재현 불가,
  프로덕션 마크업은 수정본 서빙 확인 — 실기기 전용 변수 남음
- 진단 심음(커밋 f7811ec): 설정 하단에 vp·dpr·visualViewport.scale·doc/clientWidth·
  프레임 폭 표시. 대표님에게 ① 설정 하단 진단값 ② 아이패드 사파리에서
  jejunow.vercel.app 직접 열었을 때 증상 여부 요청함 — 이 두 값이 분기점
  (사파리도 깨지면 페이지, 사파리 정상이면 셸/웹뷰)
