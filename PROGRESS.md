...
## 🏁 공모전 1차 심사(기능 심사) 제출 준비 (2026-09-04) — 마감 **09-21(월) 16:00**

- ✅ **1차 심사자료 제출 완료(2026-09-04 오전, 콘텐츠랩 접수확인 제출일자 2026-09-04로 갱신 확인)**.
  제출 내용: 팀 한예원(개인) · 제주나우 · 개요 245자 · 지역특화 예(제주특별자치도) · **유형 웹만** ·
  서비스 URL jejunow.vercel.app · 로그인 불필요 · 인증키 1세트(운영계정 신청 체크) · 활용 API 「국문 관광정보
  서비스」 · 기능설명서 PDF. 제출 버튼은 대표님이 눌렀고, 인증키도 대표님 입력(API 키는 대행 금지)
- 🔜 **Apple 승인·출시가 9/21 16:00 전에 나면 접수 페이지에서 「앱」 체크 + iOS 앱스토어 URL
  `https://apps.apple.com/kr/app/id6792661866` 추가**(기한 내 수정 가능). 콘텐츠랩 재진입은 로그인 + 이메일
  인증(투어라즈 등록 메일 windgarden419) → 접수확인 > 팀명 클릭 > 심사자료 제출 > 동의 3개 > 2단계
- ⚠ 콘텐츠랩 폼 함정: 체크박스는 프로그램 값 설정이 프레임워크 상태에 안 먹음 → 실제 클릭 필요.
  텍스트·라디오·파일은 정상. 「앱」 체크 시 마켓 URL 1건 필수

- 요건 SSOT = 운영사무국 노션 + 07-31 메일. 우리 기준 정리 = `docs/contest/1차심사_제출준비.md`,
  기능설명서 본문 초안(양식 6섹션 1:1·흐름도 mermaid 5개) = `docs/contest/기능설명서_초안.md`
- 제출처 한국관광 콘텐츠랩(api.visitkorea.or.kr, 로그인·이메일 인증). 항목: 팀 정보 / 서비스 정보 /
  테스트 정보(웹 URL + **앱은 기한 내 스토어 승인·등록 완료 링크 필수**, 로그인 불필요 선택) /
  TourAPI 인증키(인코딩·디코딩)+활용 API(KorService2: areaBasedList2·detailIntro2·detailCommon2·
  detailImage2) / **기능설명서는 제공 양식(Drive zip) 필수 + PDF 변환**
- 배점 1차: 기획 30·완성도 30·데이터 활용 20·발전성 20 + 지역특화 가점 2(제주 해당, 제주 RTO 특별상 후보)
- ⚠ FAQ: TourAPI 로컬 DB 저장·캐싱 지양(실시간 호출 권고, 호출 이력 없으면 불이익). 우리는 주간 배치
  동기화 → 대표님 결정 대기(그대로 기술 / tourapi@knto.or.kr 문의 / 상세 화면 라이브 호출 추가)
- ⚠ 앱 내 수요 데이터 출처 오기 발견·수정: "네이버 데이터랩" → "한국관광 데이터랩"(FAQ 2곳·처리방침 1곳,
...
- ✅ **Render 슬립 원인 규명·수정(09-04 10:20)**: UptimeRobot 모니터가 **2026-07-11 14:09부터 "405 Method Not
  Allowed"로 Down**(1개월 24일). 무료 플랜은 HEAD 고정(HTTP method 변경은 유료)인데 FastAPI `/keepalive`·`/health`가
  GET 전용 → 405. `api_route(methods=["GET","HEAD"])`로 수정(커밋 d39294d) → Render 자동배포 확인, 프로덕션 HEAD 200.
  UptimeRobot은 다음 5분 체크에서 Up 전환 예정. ⚠ 교훈: UptimeRobot 무료 = HEAD, 헬스 엔드포인트는 HEAD 허용 필수
- 콘텐츠랩: 대표님 로그인+이메일 인증 통과, 접수 상세 진입(팀명 한예원·개인·서비스 유형 「앱」·지역특화 예(제주)·
  제안서 jejunow_proposal.pdf). 「심사자료 제출」 1단계 = 동의 3개(개인정보·유의사항·팀원 확인) → 2단계 자료 제출.
  동의 클릭은 대표님 몫(또는 명시 지시)
- 사람 단계 잔여: 콘텐츠랩 1단계 동의 → 2단계 입력(초안 §2) → cron-job.org 가입(이중화, 선택) → 운영계정 신청(선택)
  → 승인 시 출시 버튼 → 9/21 16:00 전 제출
...
- 제출 직전 세팅(이 세션에서 처리): ① 심사 연락처 = 한예원(Yewon Han)·010-2514-3146·
  windgarden05@gmail.com + 심사 노트(공모전 출품·무계정=데모계정 불필요·위치 권한 선택적·
  네이티브 통합 논거, 영문) — ASC API appStoreReviewDetails ② **App Privacy 게시**(콘솔,
  대표님 승인) ③ **iPad 13" 세트 신설 + iPhone 6.5" 교체** — 전부 제주나우 반영 재캡처,
  API 업로드(appScreenshotSets/appScreenshots, md5 커밋) ④ 가격 = 무료 $0.00(175개국)
  ⑤ 콘텐츠 권한 = "타사 콘텐츠 포함·권한 보유"(TourAPI·카카오 지도 이용허락 근거)
- ④⑤는 「심사에 추가」 차단 항목이어서 발견 즉시 해소함
- ⚠ 리젝 대비: 4.2(웹뷰 최소 기능) 리스크 기고지 — 리젝 시 심사 노트 논거(위치·햅틱·
  백스와이프 네이티브 통합, 공모전 맥락) 기반 대응, 필요시 항소 또는 기능 보강
- ASC API 스크립트(빌드 연결·심사 정보·스크린샷 업로드·이름 변경)는 세션 스크래치에서
  실행 — 재사용 시 JWT는 admin 키 + iat 60초 백데이트(betaFeedback 조회와 동일)
...
- **TestFlight 1.0.0(21) VALID** — 실기기(빌드 21 스크린샷)로 앱 아이콘 교체·위치 팝업
  소멸 반영 확인됨
- **2차 피드백(12:38) 반영**: 홈·일정 탭 상단 여백 3rem → 0.75rem(지도·오토플랜과 정렬,
  safe-area 유지, 커밋 3cf9c71, 프로덕션 배포·HTML 실측). 웹 전용이라 재빌드 불필요 —
  앱 완전 종료 후 재실행이면 반영

## ⏭️ 다음 세션 재개점 — 계정 시스템 v1.1 (2026-08-19 기준: 코드·콘솔 전부 완료)

**현황: Phase 2(뼈대)·Phase 3(iOS 준비)·Phase 4 코드분(탈퇴·처리방침·동의 고지, 커밋
8b3e6d4)·배포 전 콘솔 단계(Supabase Site URL/Redirect·Vercel service_role env) 전부 완료.
브랜치 `feature/auth-v1_1`(master 위 rebase 유지). 남은 것 = 사람 단계뿐:**
- 심사 제출(무계정 master) 완료 후 → feature를 master에 병합·프로덕션 배포
- v1.1 제출 시 ASC App Privacy 갱신(카카오 로그인 수집 항목 반영)
- TestFlight 실기기 검증(심사 통과 후 권장)
...
**아이패드 실기기 "그대로" — 원인 확정·해소 (2026-07-23 저녁, 커밋 e9ca60e):**
- 사파리 실기기 스크린샷으로 확정: min-w-0 수정 후 넘침은 해소돼 있었고, 남은 증상은
  **2단 그리드 좌측 열이 통째로 비는 케이스**(새 세션=일정 없음 + 붐빔 없는 시각)로
  콘텐츠가 우측 반쪽에 몰린 것. 재설치 후 "그대로"도 동일(재설치→일정 소실→좌측 빔)
- 수정: `[&>div:empty]:hidden` + `:has(>div:first-child:empty)`로 빈 열 숨김+1단 접기.
  MyPlanCard 하이드레이션으로 채워지면 2단 복귀. WebKit iPad 프로파일 실측
  (fresh=1단 전폭 / 일정 있음=2단 / overflow 0)
- 진단 표시(설정 하단 ViewportDebug, 커밋 f7811ec)는 실기기 확인 후 제거 예정
- homepage 백필 완료: 802곳 중 578곳 확보(72%)
- 가로 여백 축소(커밋 ab331b8): 프레임 md 3xl·lg 5xl(세로 66px·가로 85px 여백),
  상세 main·스티키 바 동기, 일정 타임라인만 md:max-w-2xl 캡(가독). WebKit 4화면 실측
- 최종: **태블릿 풀블리드**(커밋 9e82a45, 대표님 완벽주의 반영 3차) — md:max-w-none으로
  아이패드 세로·가로 여백 0(지도 엣지·투·엣지), xl:max-w-7xl로 1280+ 데스크톱 웹데모만
  중앙 컬럼. 상세 세로 2xl 캡·일정 2xl 캡은 가독용 유지. WebKit 실측 834/1194 전폭
- ⚠ 사고 기록: PS Get/Set-Content 왕복으로 한글 소스 2파일 모지바케 → git checkout 복구.
  파일 치환은 Edit로만([[feedback-no-ps-text-roundtrip-on-utf8-source]] 메모리 등재)

**소개란 매거진 개편 (2026-07-23, 커밋 cd55379 — 감성 레퍼런스 조사 기반):**
- 조사: 스테이폴리오 매거진·여행 에디토리얼 공통 = 세리프 리드문(풀 쿼트) + 플랫
  본문 + 넉넉한 행간, 박스 카드 없음
- 첫 문장(다./요. 경계 20~160자) 세리프 리드 — **Noto Serif KR을 next/font 셀프호스팅**
  (⚠ Google Fonts CDN 링크였으면 App Privacy 실측 외부 호스트 목록이 깨짐 — 셀프호스팅이라
  런타임 호스트 불변). 본문 15px/leading-8·문단 분리·하단 페이드+계속 읽기
- WebKit 실기기 프로파일로 세리프 적용 실측(폰·아이패드)

**스팟 선택 근처 순 정렬 (2026-07-23, 커밋 0bb5501):**
- 일정 추가 시트를 추가 위치 직전 슬롯 스팟 기준 거리 오름차순으로(동선 이어붙이기).
  기준 없으면(첫 슬롯) 기존 순서. 헤딩 「OO 근처 순」+행별 km 표기, 검색 결과에도 적용
- 이미 담긴 스팟은 목록 제외(자기 자신 0.0km 1위 실측 발견 — 중복 담기도 차단)
- 실측: 성산일출봉 다음 추가 → 우뭇개해안 0.5km부터

**경로 보기 이동수단 탭 (2026-07-23, 커밋 d363b00 — 구글맵 문법):**
- 상단 아이콘 칩 자동차/도보/대중교통. 인앱 실경로는 자동차만(카카오내비 자동차 전용)
- 도보 = 직선 점선 + ×1.3 보정/4km/h 추정(추정 명시) / 대중교통 = 안내+카카오맵 연결
- 「카카오맵에서 열기」 = `kakaomap://route?...&by=모드` 딥링크, 앱 없으면 1.4초 웹 폴백
  (pagehide·visibilitychange로 앱 전환 시 폴백 취소)
- 실측: 함덕→김녕 자동차 8.6km 17분 / 도보 8.4km 2시간 6분 / 탭 전환·점선 렌더 확인.
  ⚠ 딥링크 실기기 동작(카카오맵 앱 설치 시)은 대표님 확인 필요

**오토플랜 시간 창 문제 수정 (2026-07-24, 커밋 c5d2a63):**
- 증상: 빡빡하게인데 1곳만. **진단(실데이터 플레이스루)**: 오늘 날짜면 시작=현재+1시,
  예측 20시까지라 저녁에 오늘로 짜면 창이 1~2시간뿐 → 시간 만료로 조기 종료.
  알고리즘 정상(start9=8곳, start19=2곳, start20=1곳) — 시작 시각이 유일 변수
- 수정: ① planTargets 하한을 fit로(이전 max(2,fit)이 목표2표시→실제1 불일치 유발) +
  scarceWindow 헬퍼 ② 오토플랜 질문에 **날짜 선택**(HORIZON), 오늘 목표<3이면 내일 권유
  배너, onApply가 선택 날짜를 ScheduleBuilder에 전달해 그 날짜로 저장·전환 ③ 완성 화면
  시간 만료 안내+내일 다시 짜기 / addDaysStr 순수 헬퍼
- 검증: 시뮬 648 불변식 0 / E2E 모레 빡빡하게 8곳·current=모레 / 19:30 clock 배너 렌더
- ⚠ 커밋 메시지 큰따옴표 = PS→git 인자 분리 깨짐(재발) — 메시지에 " 금지
