...
export async function fetchSpotById(spotId: number): Promise<SpotDetail | null> {
  const rows = await rest<SpotDetail[]>(
    `spots?select=spot_id,name,cat1,cat2,lat,lng,route_lat,route_lng,addr,opening_hours,image_url,is_outdoor,region,content_id,overview,tel,homepage&spot_id=eq.${spotId}`,
    3600,
  );
